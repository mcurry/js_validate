 ...
 * @license     MIT
 */
(function($){var options=null;$.fn.validate=function(rules,opts){options=$.extend({},$.fn.validate.defaults,opts);$.each(opts.watch,function(fieldId){$("#"+opts.watch[fieldId]).change(function(){$.fn.validate.ajaxField($(this));});});return this.each(function(){$this=$(this);$this.submit(function(){var errors=[];var val=null;$.fn.validate.beforeFilter();$.each(rules,function(field){$field=$("#"+field);if($field.attr("type")=="checkbox"){if($field.filter(":checked").length>0){val=$field.filter(":checked").val();}else{val="0";}}else{val=$field.val();}
var fieldName=$field.attr('name');if(typeof val=="string"){val=$.trim(val);}
...
return true;};$.fn.validate.notEmpty=function(val,params){if(typeof val=="string"&&val==""){return false;}
if(typeof val=="object"&&val.length==0){return false;}
return true;}
$.fn.validate.range=function(val,params){if(val<parseInt(params[0])){return false;}
